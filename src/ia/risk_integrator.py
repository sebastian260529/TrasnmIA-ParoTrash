...
from src.ia.pattern_analysis import predict_risk_from_patterns, load_alert_dataset
from src.ia.whatsapp_tm_service import (
...
from src.geo.location_service import (
...
def _score_buses(db: BusDatabase, zona: str) -> Dict[str, Any]:
    try:
        ...
        buses_detenidos = resumen.get("buses_sin_movimiento", 0)
        buses_lentos = resumen.get("buses_lentos", 0)
            ...
            for bus in captura_real:
                ...
                if lat is not None and lon is not None:
                    try:
                        ...
                        pass
        total_afectados = buses_detenidos + buses_lentos + buses_cercanos
        if len(captura_real) == 0 and total_afectados == 0:
            return {
                "score": 0,
                "tipo_dato": "sin_datos",
        ...
        if total_afectados == 0:
            score = 0
            detalle = f"No se detectaron buses con señales de afectacion cerca de {zona}. Se revisaron {len(captura_real)} buses reales."
        elif total_afectados <= 3:
            score = 25
            detalle = f"Se detectaron {total_afectados} buses con señales leves cerca de {zona}."
        elif total_afectados <= 7:
            score = 55
            detalle = f"Se detectaron {total_afectados} buses con velocidad baja o agrupados cerca de {zona}."
        else:
            score = 80
            detalle = f"Se detectaron {total_afectados} buses con velocidad baja/agrupados cerca de {zona}. Alta probabilidad de afectacion."
        return {
            "score": min(score, 95),
            "tipo_dato": "real",
            "detalle": detalle,
            "cantidad": total_afectados,
...
def _score_firebase_reportes(zona: str) -> Dict[str, Any]:
    try:
        from src.firebase.client import get_all_reportes
