...
def clustering_por_direccion(buses: List[Dict], radio: int = None) -> Dict[str, List[Dict]]:
    """
    Agrupa buses primero por dirección y luego por proximidad espacial.
    Devuelve: {"ARRIBA": [clusters], "ABAJO": [clusters]}
    """
    radio = radio or RADIO_ANOMALIA
...
def detectar_manifestaciones(buses_detenidos: List[Dict]) -> List[Dict]:
    """Detecta posibles manifestaciones usando clustering por dirección."""
    if len(buses_detenidos) < 5:
        return []
    
    clusters_por_direccion = clustering_por_direccion(buses_detenidos, RADIO_ANOMALIA)
    resultados = []
...
def detectar_trancones(buses_lentos: List[Dict]) -> List[Dict]:
    """Detecta trancones usando clustering por dirección."""
    if len(buses_lentos) < 3:
        return []
    
    clusters_por_direccion = clustering_por_direccion(buses_lentos, RADIO_ANOMALIA)
    resultados = []
